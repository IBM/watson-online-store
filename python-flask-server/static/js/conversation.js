...
var ConversationPanel = (function() {
  ...
  function init() {
    chatUpdateSetup();
    // Api.sendRequestUsingSocket('', null );
    setupInputBox();
  }
...
// Set up the input box to underline text as it is typed
  ...
  function displayMessage(newPayload, typeValue) {    
    ...
    var dataExists = newPayload.data || newPayload.image ? true : false;   
    if (isUser !== null && dataExists) {
        ...
        if(currentText.image){
          var messageJson = {
            ...
            'children': [{
              ...
              'tagName': 'p',
              'text': '<img src=\''+currentText.image+'\' class=\'thumbnail\'/>',              
            }]
          };          
        } else {
        data = currentText.data.replace(/`/g, '')
        data = data.replace(/\*/g, '')
        messageJson = {
          // <div class='segments'>
          'tagName': 'div',
  ...
  function inputKeyDown(event, inputBox) {
    ...
    if (event.keyCode === 13 && inputBox.value) {
      ...
      Api.sendRequestUsingSocket(inputBox.value, context);
